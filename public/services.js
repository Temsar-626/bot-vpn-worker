...
async function studioServices() {
  ...
  return `<div class="sv-workspace"><div class="sv-header"><div><span class="v-eyebrow">SERVICE COMMERCE / CLOUDFLARE</span><h3>${L("مرکز فروش و مدیریت سرویس", "Service management center")}</h3><p>${L("مینی‌اپ مشتری، کیف پول و ساخت سرویس با ثبت مالی هماهنگ", "Customer portal, coordinated wallet accounting and service provisioning")}</p></div><div class="flex items-center gap-2 flex-wrap">${svBtn(L("باز/بستن همه بخش‌ها", "Expand / collapse all"), "secAll")}<span class="v-badge good">${L("ذخیره‌سازی متصل آماده است", "Credential store ready")}</span></div></div><nav class="v-tabs sv-tabs">${SV_TABS.map(([id, icon, label]) => `<button type="button" class="v-tab ${SV.tab === id ? "active" : ""}" data-act="svTab" data-tab="${id}">${vIcon(icon)}${label[S.lang === "en" ? 1 : 0]}</button>`).join("")}</nav><div id="sv-body"><div class="v-skeleton"></div></div></div>`;
}
...
ACTIONS.svStockImport = () =>
  vModal(
    ...
    vSelect(
      ...
      vNote(
        L(
          "برای WireGuard چندخطی، گزینه زیر را فعال کنید.",
          "For a multiline WireGuard file, enable the option below.",
        ),
      ) +
      vCheck(
        "ss-multiline",
