...
const actions = {
  ...
  simple: async (d) => {
    if (
      ["revoke", "disable", "cancel_refund"].includes(d.kind) &&
      !confirm(
        T(
  ...
  cancelBpb: async (d) => {
    const q = await api("/services/" + d.id + "/cancel-quote");
    modal(
      T("لغو فوری و استرداد", "Instant cancel & refund"),
      `<p>${T("روزهای باقی‌مانده", "Remaining days")}: <b>${num(q.remainingDays)} / ${num(q.totalDays)}</b></p><p>${T("مبلغ بازگشتی به کیف پول", "Wallet refund amount")}: <b>${money(q.amount)}</b></p>${notice(T("پس از لغو، لینک اشتراک از کار می‌افتد و روز جاری کسر می‌شود.", "After cancellation the subscription stops working; the current day is charged."), "warn")}<div class="actions">${button(T("✅ تأیید لغو", "Confirm cancel"), "simple", `data-id="${d.id}" data-kind="cancel_refund"`, "danger")}</div>`,
    );
  },
  gift: () =>
    modal(
